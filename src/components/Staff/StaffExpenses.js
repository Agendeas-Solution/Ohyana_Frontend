...
import { GetExpenseList, PaymentStatusUpdate, StatusUpdate } from '../../services/apiservices/staffDetail';
const StaffExpenses = () => {
            ...
            <Box className="target_section">
                <Box className="attendance_data_row col-md-12">
                    <Box className="total_expenses_data days_data  col-md-2">
                        <Typography sx={{ fontSize: "15px" }} variant="span">Approved</Typography>
                        ...
                        <TableBody>
                            {
                                expenseList && expenseList.map((row) => {
                                    return <TableRow>
                                        <TableCell>{moment(row?.date).format("D/MM/YY")}</TableCell>
                                        <TableCell align="left">{row?.name}</TableCell>
                                        <TableCell align="left">{row?.amount}</TableCell>
                                        <TableCell align="left">{row?.status === "APPROVED" ? row?.aprrovalAmount : row?.status}</TableCell>
                                        ...
                                        <TableCell align="left" className="d-flex flex-row">
                                            {row?.status === "APPROVED" || "REJECTED" ? row?.status
                                                :
                                                <>
                                                    <Button
                                                        onClick={() => handleStatusUpdate(row?.id, "APPROVED")}
                                                        className="common_button">Approve</Button>
                                                    <Button
                                                        onClick={() => handleStatusUpdate(row?.id, "REJECTED")}
                                                        className="common_button">Reject</Button>
                                                </>}
                                        </TableCell>
                                        <TableCell align="right">
                                            {row?.payment_status === "DONE" ? <Typography>-</Typography> : <Button onClick={() => handlePaymentStatusUpdate(row?.id)} className='common_button'>Update</Button>}
                                        </TableCell>
                                    </TableRow>
                                })
                            }
