...
const AppointmentList = props => {
  ...
  const { setEditAppointmentDialogFlag } = useContext(
    ContextEditAppointmentDialog,
  )
  const [AppointmentDetails, setAppointmentDetails] = useState({})
  const { successSnackbar } = useContext(ContextSnackbar)?.state
  const { setSuccessSnackbar } = useContext(ContextSnackbar)
  const [deleteAppointmentDialogControl, setDeleteAppointmentDialogControl] =
    useState({
      status: false,
      id: null,
    })
      ...
      res => {
        if (res.success) {
          setDeleteAppointmentDialogControl({
            ...deleteAppointmentDialogControl,
            status: false,
          })
  ...
  return (
    <>
      ...
      {editAppointmentDialogFlag === true && (
